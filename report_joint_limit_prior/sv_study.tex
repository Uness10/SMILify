...
\emph{clipping bound}: the ROM the starting checkpoint \armbase{} would have if
its trajectory were simply clipped into the authored box,
\begin{equation}
...
\armbase{} is set apart: it did not receive the additional epochs. The
plausibility columns of \armref{} were computed on the cluster
(\texttt{prior\_study\_results\_matched/\allowbreak sv\_reference/\allowbreak analysis/}) but are not
yet included here; plausibility comparisons below are therefore made against
\armbase{}.}
\label{tab:main}
...
\midrule
\armref{}        & 0         & 433 & --- & --- & --- & --- & \best{0.9054} & \best{0.7439} & \best{0.5110} & \best{0.9943} & \best{6.369} \\
...
plausibility improves and accuracy degrades with every decade. The open marker
joined by a dashed segment is \armbase{}, which did not receive the $49$
additional epochs; the figure predates the epoch-matched control, whose MPJPE
(\SI{0.9054}{\milli\metre}) lies below every point shown.}
\label{fig:tradeoff}
\end{figure}
...
\caption{The twelve axes most frequently violated by \armbase{}, and
...
predominantly coxa and femur flexion. \armbase{} violates
...
into the authored box. The ROM of the epoch-matched \armref{} is not yet
included (see \Cref{sec:res-rom}).}
\label{tab:rom}
\begin{tabular}{l r *{5}{r}}
\toprule
Joint group & axes & \armbase{} & $10^{-4}$ & $10^{-3}$ & $10^{-2}$ & $10^{-1}$ \\
\midrule
...
All ROM figures in this section are relative to the starting checkpoint
\armbase{}. The pose analysis of the epoch-matched \armref{} has been computed
on the cluster but not yet merged into these tables; until it is, the ROM
changes below combine the prior with $49$ epochs of continued training, as the
original design did. \Cref{sec:res-centre} gives reasons to expect the
continuation to act mainly on the centre of the pose distribution rather than
its spread, but this should be checked against \armref{}.

\armbase{} moves through a mean ROM of \SI{66.1}{\degree} per bounded
...
\armbase{} trajectory into the box would leave \SI{37.0}{\degree}. The trained
...
\caption{ROM retained relative to \armbase{}, with axes grouped by
\armbase{}'s own violation rate on that axis. The dashed rule in each group is
...
\armbase{} retain \SIrange{93.6}{95.8}{\percent} of their range at every
...
\SI{62.3}{\percent} of the \armbase{} range where clipping would retain
...
ROM and authored range. \armbase{} (mean $1.95$, median $1.18$; $79$ axes
...
effects are therefore separable in principle. The epoch-matched \armref{}
allows this to be tested directly: if the reading is right, its centre shift
relative to \armbase{} should be close to \SI{4.2}{\degree} and its ROM close
to that of \armbase{}.
...
($+\SI{11.3}{\percent}$), and ROM close to the clipping bound on chronic axes.
...
\SI{74}{\percent} of femur range, $+\SI{15.7}{\percent}$ MPJPE and
$+\SI{26.2}{\percent}$ mean 2D error relative to \armref{}.

\subsection{Threats to validity}

Three caveats bound the conclusions. First, the epoch-matched control is
matched on accuracy but not yet on plausibility: its violation and ROM figures
have been computed but are not included, so the plausibility and ROM
comparisons still use \armbase{}. Second, \armref{} is scored at the
...
\item Merge the pose analysis of the epoch-matched control
      (\texttt{prior\_study\_results\_matched/\allowbreak sv\_reference/\allowbreak analysis/}) into
      \Cref{tab:main,tab:rom}, and regenerate the figures with \armref{} as
      the reference.
